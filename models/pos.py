...
class PosSession(models.Model):
    ...
    def pos_side_get_session_amount_data(self, session_id):
        ...
        for pos_order in pos_order_ids:
            ...
            for line in pos_order.lines:
                ...
                for categ in line.pos_categ_ids:
                    if categ.name:
                        if categ.name in sold_product:
                            sold_product[categ.name] += line.qty
                        else:
    ...
    def _loader_params_pos_session(self):
        ...
        result['search_params']['fields'].extend(['name','id','state'])
        return result
    ...
    def load_pos_data(self):
        ...
        session_data = self._get_pos_ui_custom_pos_session(self._loader_params_custom_pos_session())
        loaded_data['custom_sessions'] = session_data
        return loaded_data

    def _get_pos_ui_custom_pos_session(self, params):
        custom_sessions = self.env['pos.session'].sudo().search_read(**params['search_params'])
        print("custom_sessions",custom_sessions)
        return custom_sessions


    def _loader_params_custom_pos_session(self):
        return {
            'search_params': {
                'domain': [],
                'fields': [
                    'id', 'name','state', 'cash_register_balance_start', 'cash_register_balance_end_real', 'cash_register_difference'],
            },
        }